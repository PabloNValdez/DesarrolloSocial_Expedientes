    ...
    */
    ...
    public static function index()
    {
        $expediente = DB::table('expedientes')
                        ...
                        ->select(   'expedientes.id as id',
                                    ...
                                    'expedientes.nro_expediente',
                                    'extractos.descripcion as extracto',
                                    DB::raw('CONCAT (DATE_FORMAT(expedientes.fecha, "%d-%m-%Y")," ", DATE_FORMAT(expedientes.created_at, "%H:%i" )) as fecha_creacion'),
                                ...
                                ->orderBy('expedientes.fecha', 'DESC')
                                ->orderBy('expedientes.nro_expediente', 'DESC')
                                // ->limit(1000)
    ...
    public static function nroExpediente($año_exp)
    {
        $expedientes =  DB::table('expedientes')
                        ...
                        ->where('nro_expediente', 'LIKE', '%/'.$año_exp)
                        ->where('nro_expediente', 'NOT LIKE', '800-8%')
                        ->where('nro_expediente', 'NOT LIKE', '800-9%')
                        ->where('nro_expediente', 'NOT LIKE', '800-1%')
                        ->orderByDesc('nro_expediente')
                        ->take(1)
                        ->get()
                        ;
        if(count($expedientes) > 0){
            $nuevo = substr($expedientes->first()->nro_expediente, 4);
            $nuevo = substr($nuevo, 0, stripos($nuevo,'/'));
            $cont = $nuevo + 1;

        }else{
            $cont = $expedientes->count() + 1;
        }
        $nro_aleatorio = str_pad($cont,5,"0000",STR_PAD_LEFT);
        // $nro_aleatorio = str_pad($cont,5,"025",STR_PAD_LEFT);
        // $cont = $expedientes->count() + 1;
        // $nro_aleatorio = str_pad($cont,5,"025",STR_PAD_LEFT);
        ...
        $historial_ultimo_movimiento = DB::table('historiales')
                        ->select('expedientes.id as expediente_id',
                                ...
                                DB::raw('CONCAT (DATE_FORMAT(expedientes.fecha, "%d-%m-%Y")," ", DATE_FORMAT(expedientes.created_at, "%H:%i" )) as fecha_creacion'),
                                 ...
                                 'extractos.descripcion as extracto',
                                 DB::raw("DATE_FORMAT(expedientes.fecha , '%d-%m-%Y') as fecha_env"),
