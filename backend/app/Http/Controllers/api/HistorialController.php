...
class HistorialController extends Controller
{
    ...
    */
    public function updateEstado(Request $request)
    {
        ...
        $user = Auth::user();
        $expediente = Expediente::findOrFail($request->expediente_id);
        // $ultimaFoja= Historial::where('expediente_id', $expediente->id)->orderByDesc('created_at')->take(2)->get();
        // dd($ultimaFoja);
        ...
        */
        if ($request->estado_expediente == 3 or $request->estado_expediente == 5) {
            ...
            if ($request->estado_expediente == 5)
            {
                ...
                // if ($ultimaFoja->count() == 2) {
                    $expediente->fojas = $ultimaFoja->last()->fojas;
                    $historial->fojas = $ultimaFoja->last()->fojas;
                // }
                $expediente->save();
            }
        }
    ...
    public function historialExpediente(Request $request)
    {
        $historiales = DB::table('historiales')
                         ...
                         ->get([
                             ...
                             'historiales.observacion as observacion',
                             ...
                             DB::raw("DATE_FORMAT(historiales.created_at, '%d-%m-%Y %h:%i') as fecha"),
                             'historiales.motivo as motivo',
                             DB::raw("DATE_FORMAT(historiales.created_at, '%h:%i') as hora"),
                            ]);
